...
import { tempFill } from '../lib/tempScale'
...
export function FranceMap({ regions, selectedId, onSelect }: Props) {
  return (
    <svg
    ...
    >
      ...
      {regions.map((r) => {
        ...
        return (
          <g key={r.id} className="map-marker" onClick={() => onSelect(r.id)} style={{ cursor: 'pointer' }}>
            ...
            />
            <title>
              {r.name}: growing season {grow != null ? `${grow.toFixed(1)} °C` : 'n/a'}
            </title>
            <text
              x={x}
              y={y + rRadius + 12}
      ...
      <g className="map-legend" transform="translate(12, 380)">
        <text fontSize="10" fill="#666">
          Growing season (Apr–Oct) °C — cooler → warmer
        </text>
        {[0, 1, 2, 3, 4].map((i) => (
          <rect
            key={i}
